  ...
  public function subcategory($slug1,$submenu){
    ...
    //    return  $detailspage;
        ...
        if($subcategory_type == "Group News"){
            ...
            // return $newsevents;
            return view("website.newsevents")->with(['newsevents'=>$newsevents,'jobs'=>$jobs,'menus'=>$menus,'sliders'=>$sliders,'about'=>$About,'global_setting'=>$global_setting,'slug_detail'=>$slug_detail,'slug1'=>$slug1,'slug2'=>$slug2]);
    ...
    public function FullStoryRead($slug){
        ...
        $newsevents = Navigation::where('page_type',"Group News")->get()  ;
        // $news =  $newsevents->childs;
        // return "hi";    
        // return $slug ;
        $global_setting = GlobalSetting::all()->first(); 
        $menus = Navigation::query()->where('nav_category','Main')->where('page_type','!=','Job')->where('page_type','!=','Photo Gallery')->where('page_type','!=','Notice')->where('parent_page_id',0)->where('page_status','1')->orderBy('position','ASC')->get();
        return view("website.detailspage")->with(['detailspage'=>$detailspage,'newsevents'=>$newsevents,'menus'=>$menus,'global_setting'=>$global_setting,'job_slug'=>$slug,'slug2'=>$detailspage]);
    }
